...
import de.mfo.jsurfer.algebra.ColumnSubstitutor;
import de.mfo.jsurfer.algebra.ColumnSubstitutorForGradient;
import de.mfo.jsurfer.algebra.UnivariatePolynomial;
import de.mfo.jsurfer.algebra.UnivariatePolynomialVector3d;
import de.mfo.jsurfer.rendering.LightProducts;
import de.mfo.jsurfer.rendering.LightSource;
import de.mfo.jsurfer.rendering.RenderingInterruptedException;
...
public class RenderingTask implements Callable<Void>
{
	...
	switch (dcsd.antiAliasingPattern)
	{
		...
		for (int y= 0; y < internal_height; y++)
		{
			...
			//			Main2.logger.log(Level.INFO, "u:" + u + " v:" + v/* + " scs:" + scs + " gcs:" + gcs*/);
			int rgb= tracePolynomial(scs, gcs, u, v).get().getRGB();
			dcsd.colorBuffer[dcsd.width * (yStart + y) + xStart + x]= rgb;
			//			Main2.logger.log(Level.INFO, "rgb:" + rgb);
