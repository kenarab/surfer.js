...
public class XYZPolynomial
{
    ...
    */
    ...
    /*
        ...
        public XYZPolynomial substitute( XYZPolynomial xPoly, XYZPolynomial yPoly, XYZPolynomial zPoly )
        {
            ...
            ArrayList< Term > result_terms = new ArrayList< Term >();
            for( Term t : this.terms )
            {
        ...
        static Term[] multinomialExpansion( Term[] terms, int exp )
        {
            ...
            for( int i = 0; i <= exp; ++i )
            {
                int exp_i = MultinomialCoefficients.binomialCoefficient( exp, i );
                for( int j = 0; j <= i; ++j )
                {
                    int i_j = MultinomialCoefficients.binomialCoefficient( i, j );
                    for( int k = 0; k <= j; ++k )
                    {
                        int j_k = MultinomialCoefficients.binomialCoefficient( j, k );
    ...
    */
	...
	//System.out.println( "last_t=" + last_t.intToString() );
	//System.out.println( "z_result=" + z_result );
	//System.out.println( "y_result=" + y_result );
	//System.out.println( "x_result=" + x_result );
