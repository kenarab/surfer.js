...
public abstract class TableBulkRenderer
{
	 ...
	 */
	public abstract void renderCell(int row, int column, Object cellData, StringBuffer accum);
    }
     ...
     */
    public final void renderRows(Iterable<? extends Iterable> rows, RendererCallback callback)
    {
     ...
     */
    public final void renderRows(TableModel<?> tableModel, int startRow, int numRows, RendererCallback callback)
    {
	RenderingOptions options= createRenderingOptions();
     ...
     */
    protected void renderRow(int rowIndex, Iterator<Object> row, StringBuffer buffer, final RenderingOptions options)
    {
	buffer.append("<tr>");
     ...
     */
    protected void renderRows(final Iterator<Iterator<Object>> rows, final RenderingOptions options)
    {
	...
	class RenderTableCommand implements IncrementalCommand
	{
		...
		for (; rows.hasNext(); ++rowIndex)
		{
			...
			if (time > endSlice)
			{
			    ...
			    if (time > endTime)
			    {
				throw new IllegalStateException("Cannot bulk render a table which takes more than " + MAX_TIME + " milliseconds to render");
			    }
			    return true;
			}
		    }
	...
	TableModel.Callback requestCallback= new TableModel.Callback()
	{
	    ...
	    public void onRowsReady(Request request, final TableModel.Response response)
	    {
