...
public class GWTCanvasImplIE6 implements GWTCanvasImpl
{
    ...
    private static native void ensureNamespace(Document doc, String prefix, String urn) /*-{
    ...
    {
	pathStr.push(PathElement.arc(x, y, radius, startAngle, endAngle, anticlockwise, this));
    }
    ...
    public void drawImage(ImageElement img, double sourceX, double sourceY, double sourceWidth, double sourceHeight, double destX, double destY, double destWidth, double destHeight)
    {
    ...
    {
	int coordX= doubleToFlooredInt(Math.floor(10 * (matrix[0] * x + matrix[1] * y + matrix[2]) - 4.5f));
    ...
    {
	int coordY= doubleToFlooredInt(Math.floor(10 * (matrix[3] * x + matrix[4] * y + matrix[5]) - 4.5f));
	// record current point to derive bounding box of current open path.
